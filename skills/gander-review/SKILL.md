...
3. Record durable review state as you go:
   ...
   Anchor comments/tasks to the smallest useful location. Use `--general` only
   for session-level comments with no location/excerpt. Commands accept compact
   unique prefixes for comment/task IDs (`<comment-prefix>`, `<task-prefix>`;
   ...
   optional higher-level work as tasks when useful, but do not rename tasks to
   action items; action items are the handoff selection (open tasks + todo
   comments). Use walkthroughs for reading order or handoff context.
...
4. Export a handoff when asked or when another agent will act:
   ...
   gander handoff --mode delegate --task <task-prefix> --include-comment <todo-comment-prefix> --format json
   ```
