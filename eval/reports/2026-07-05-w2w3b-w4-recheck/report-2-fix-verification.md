...
4. **[nit] Chooser rows that match the fuzzy filter only weakly stay listed.** With filter `main`, `refactor: extract retry policy from worker` and `feat: priority scheduling…` remain as candidates (presumably subsequence matches across the row text). Combined with finding 2 this amplifies the mis-selection risk; on its own it is just noise.
